...
export default function Home() {
	const [services, setServices] = useState<ServiceStatus[]>(SERVICES.map((s) => ({ ...s, status: "CHECKING" as const })));
			...
			<div className="relative flex items-center justify-center min-h-screen px-4 py-8">
				<div className="w-full max-w-2xl font-mono">
					...
					<div className="mb-8 p-4 bg-primary/5 border border-primary/20 rounded text-xs space-y-2">
						...
						<div className="text-primary">Network Name: spider</div>
						<div className="text-primary">{`Services: ${services.filter((s) => s.status === "ONLINE").length}/${services.length} ONLINE`}</div>
						...
						{services.map((service, index) => (
							...
							>
								<div className="flex items-center gap-3 text-sm sm:text-base">
									<span className="text-primary group-hover:text-accent transition-colors">{focusedIndex === index ? ">" : " "}</span>
